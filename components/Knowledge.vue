<template>
   <div class="bg-gray-300 h-full">
    <div class="pt-28   text-center">
      <p class="text-3xl text-cyan-600">You can search hear WebShows</p>
    </div>
     <div>
    <form @submit.prevent="submit" class="flex justify-center">
      <input type="text" class="mt-10 h-10 p-3   w-80 rounded-xl focus:bg-amber-100 items-center" v-model="search" />
      <button class="bg-slate-400 hover:scale-105 duration-500 hover:bg-blue-600 hover:text-white text-x rounded-md ml-2 h-10 w-20  mt-10 p-1">Search</button>
    </form>

    <div class="grid grid-flow-row grid-cols-3 w-auto mt-10">
